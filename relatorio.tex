...
\begin{document}
    ...
    A operação de Hash inlcui apenas um número constante de operações básicas. Assim, essa função $T(k) \in \Theta(1)$. Porém, analisaremos também a esperança de elementos por bucket, que será usada para a análise da complexidade do caso médio de outras operações.
    \subsubsection{Complexidade de \texttt{createHashMatrix}}
    Nessa operação, alocamos memória de acordo com o Struct \texttt{hashMatrix}, o que tem custo assumido como constante. Então, atribuímos valores iniciais para os 5 campos da Struct, cada atribuição em tempo constante. Então retornamos a matriz. $T  (k) \in \Theta(1)$. 
    \subsubsection{Complexidade de \texttt{resize}}
    Após algumas operações de custo constante, percorremos o vetor de espalhamento, e para cada elemento do vetor, percorremos o bucket associado a ele. Como sabemos que 
    \subsubsection{Complexidade de \texttt{getElement}}
    a
    \subsubsection{Complexidade de \texttt{setElement}}
    a
    \subsubsection{Complexidade de \texttt{matrixMultiplication}}
    a
    \subsubsection{Complexidade de \texttt{matrixAddition}}
    a
    \subsubsection{Complexidade de \texttt{matrixScalarMultiplication}}
    
    \subsubsection{Complexidade de \texttt{transpose}}
